...
| 1 首次使用 | 未开始 | Onboarding 三步、空日历 | 新用户建好餐厅，落到空日历 |
| 2 记录闭环 | 未开始 | 默认 Record Shift、实时计算、日历绿色金额 | 点日期记一班并在日历看见（MVP，此后可自用） |
| 3 当天详情 | 未开始 | Day Details、同日多班、编辑、删除确认与 Undo | 查看、改、删当天班次 |
| 4 表单进阶 | 未开始 | More options、计算说明、校验、无餐厅拦截、跨午夜 | 默认表单仍短，复杂规则可展开 |
